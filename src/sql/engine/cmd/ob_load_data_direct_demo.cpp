...
int ObLoadSequentialFileReader::read_next_buffer(ObLoadDataBuffer &buffer)
{
  ...
  if (OB_UNLIKELY(!file_reader_.is_opened())) {
    ...
    if (OB_FAIL(file_reader_.pread(buffer.end(), buffer_remain_size, offset_, read_size))) {
      ...
      char term = term_[0];
      while (*(buffer.end() + read_size - 1) != term) {
        read_size--;
      }
      offset_ += read_size;
...
int ObLoadDataDirectDemo::inner_init(ObLoadDataStmt &load_stmt) {
  ...
  // init file_reader_
  else if (OB_FAIL(file_reader_.open(
               load_args.full_file_path_,
  ...
  for (int i = 0; i < MAX_THREAD_NUMBER; i++) {
    if (OB_FAIL(csv_parser_[i].init(load_stmt.get_data_struct_in_file(),
...
void ObLoadDataDirectDemo::start_thread(int id) {
  int ret = OB_SUCCESS;
